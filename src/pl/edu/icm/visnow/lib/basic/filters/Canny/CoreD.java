...
public class CoreD extends CoreBase {
    ...
    protected float calc3MagSqr(int x, int y, int z) {
        ...
        dy = (float)(
                in[z1 + y1 + x1] + 2.*in[z1 + y1 + x] + in[z1 + y1 + x2]
    ...
    protected float[] calc2Grad(int x, int y) {
        ...
        int x1 = x == 0 ? 0 : x - 1;
        int x2 = x == w - 1 ? w - 1 : x + 1;
        dout[ya + x] = (float)(in[y1 + x1] - in[y1 + x2]
                + 2.*(in[ya + x1] - in[ya + x2])
                + in[y2 + x1] - in[y2 + x2]);
        dout[ya + x] = (float)(in[y1 + x1] + 2.*in[y1 + x] + in[y1 + x2]
